...
It would seem that the linear model going forward and in both direction have greater explanatory power. They score lowest on the AIC and perform best both on the training and test data.
...
Prediction is done using the baseline model to establish a baseline RMSE. Next the random forest is tuned and a new model is developed. Again prediction is done and a RMSE is calculated to see if the model improves.
...
personality_train_ex3_woeasy <- dplyr::select(personality_test_ex3, -easygon)
personality_test_ex3_woeasy <- dplyr::select(personality_test_ex3, -easygon)
...
baseline_predictions <- predict(baseline_rf, newdata = personality_train_ex3_woeasy)
...
personality_rf_tune <- tuneRF(y = personality_train$easygon, x = dplyr::select(personality_test_ex3, -easygon), improve = 0.001)
...
tuned_predictions <- predict(final_rf_mtry, newdata = dplyr::select(personality_test_ex3, -easygon))
...
personality_tune_rf_one <- function(n){
  output <- tuneRF(y = personality_train$easygon, x = dplyr::select(personality_test_ex3, -easygon), 
...
```{r}
# Create a confusion matrix
confusion_matrix <- table(predicted = tuned_predictions, actual = personality_test$easygon)
print(confusion_matrix)

```

SOMETHING HAPPEND ALONG THE LINE AND THE RESULTS CHANGED, so the below text is based on the original results.
...
The tables shows the RMSE scores for all used models. The list is sorted beginning with the lowest score.

From this it can be concluded that the linear models forward and both direction have the lowest RMSE score of 1.35, indicating greater explanatory power compared to the other models.

This is followed by the tuned random forest model, with a score of 1.42.

The linear intercept model performs worst, which is not unexpected.

Using the same data to train multiple models, refining them to the extend possible and the juxtaposing them helps us compare them and understand. From this we are able to say something about each model's performance compared to one another.
...
Time ran out, so not all analysis are performed. I focused on logistic regression and LDA, as they had not been performed in section I.
