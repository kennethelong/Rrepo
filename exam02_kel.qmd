...
For the random forest model, firstly a model is developed using randomForest trained on the training data to create a base line.
...
\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_Under construction
...
```{r}
## Test
## personality_rf_predictions <- predict(personality_rf_tune, newdata = select(personality_test_ex3, -easygon), type = "class")

personality_rf_predictions <- predict(
  personality_rf_tune, 
  newdata = select(personality_test_ex3, -easygon)
)
personality_rf_predictions <- predict(personality_rf_tune, newdata = select(personality_test_ex3, -easygon))
...
```{r}
## personality_rf_predictions <- predict(personality_RF_tune, newdata = personality_test)
personality_rf_predictions <- predict(personality_rf_tune, newdata = personality_test_ex3)
personality_rf_predictions_numeric <- as.numeric(as.character(personality_rf_predictions))
actual_numeric <- as.numeric(as.character(personality_test_ex3$easygon))

model_rmse_rf_pruned <- rmse(actual_numeric, personality_rf_predictions_numeric)

print(model_rmse_random_tree)
```
