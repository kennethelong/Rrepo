...
pop_long <- ex8_wpop |> pivot_longer(cols = '1960':'2022', names_to = "year", values_to = "Population" )
...
## Building a ggplot graph NOT DONE
ggplot(data = ex9_wpop, mapping = aes(year, Population, group = 'Country Name')) + 
  geom_point()+ theme_minimal() +
  ##geom_smooth() +
  geom_line(alpha = 0.1)

ex9_wpop_final <- ex9_wpop <- ggplot(ex9_wpop, aes(x = year, y = pop, group = 'Country Name')) +
  geom_line(alpha = .1) +
  labs(x = "Year", y = "Population")

ggplot(data = ex9_wpop, mapping = aes(x = year, y = Population, group = 'Country Name')) +
  geom_line(alpha = .1)
##+   labs(x = "Year", y = "Population")

ex9_wpop_final

ex11_pl_pop <- ggplot(pop_index, aes(x = year, y = pop, group = country)) +
  geom_line(alpha = .1) +
  labs(x = "Year", y = "Population")
...
## No scaling needed
## ex21_growth_sacled <- scale(ex21_growth)
## ex21_growth_dvec <- dist(ex21_growth_sacled, method = "euclidean")
...
## Calculating distance
ex24_aggr_growth_long_dvec <- dist(ex24_aggr_growth_long, method = "euclidean")
...
## Using 9 clusters to organize
clusters <- c(9)
ex24_cl <- cutree(ex24_aggr_growth_long_hc, k=clusters) |> as.data.frame()
ex24_cl |> head(5)
...
ex24_growth_clust <- ex24_cl |> mutate(aggr_growth_long)
